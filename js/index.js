//  API Key for news : b7bd82b703384388ad7d506896a50c20;

let apiKey = 'b7bd82b703384388ad7d506896a50c20';

// grab the news container
let newsCard = document.getElementById('newsCard');

// create a ajax get request

const xhr = new XMLHttpRequest();

xhr.open('Get',`https://newsapi.org/v2/top-headlines?country=in&apiKey=${apiKey}`,true);
// xhr.open('Get',`https://api.spoonacular.com/food/products/search?query=yogurt&apiKey=96cc52ef971642f6a5821aa33e44498a`,true);



xhr.onload = function(){
     if(this.status === 200){
          let newsObj = JSON.parse(this.responseText);
          let newsHtml ='';
          // console.log(newsObj)
          console.log(newsObj.articles)
          for(let key in newsObj.articles){
               if(newsObj.articles[key].description === null){
                    newsObj.articles[key].description = 'Please click on link to';
                    
               }if(newsObj.articles[key].urlToImage === null){
                    newsObj.articles[key].urlToImage = 'data:image/jpeg;base64,'
               }
               if(key == 0){
                    newsHtml +=    `<div class="card mt-3">
                                        <div class="card-header" id="heading${key}">
                                             <h2 class="mb-0">
                                                  <button class="btn btn-block text-left collapsed" type="button" data-toggle="collapse" data-target='#collapse${key}' aria-expanded="false" aria-controls="collapse${key}">
                                                       ${newsObj.articles[key].title}
                                                  </button>
                                             </h2>
                                        </div>

                                        <div id="collapse${key}" class="collapse show" aria-labelledby="heading${key}" data-parent="#newsCard">
                                             <div class="card-body">
                                                  <img src="${newsObj.articles[key].urlToImage}" alt="news photo">
                                                  <h1 class="mx-3"> ${newsObj.articles[key].title}</h1>
                                             </div>
                                             <p>${newsObj.articles[key].description}
                                             <a href="${newsObj.articles[key].url}" target="_blank">View Ful Coverage</a></p>
                                             <h5 style="font-weight:600">News By : ${newsObj.articles[key].source.name}</h5>
                                        </div>
                                   </div>`;
               }else{
                    newsHtml +=    `<div class="card mt-3">
                                        <div class="card-header" id="heading${key}">
                                             <h2 class="mb-0">
                                                  <button class="btn btn-block text-left collapsed" type="button" data-toggle="collapse" data-target='#collapse${key}' aria-expanded="false" aria-controls="collapse${key}">
                                                       ${newsObj.articles[key].title}
                                                  </button>
                                             </h2>
                                        </div>

                                        <div id="collapse${key}" class="collapse" aria-labelledby="heading${key}" data-parent="#newsCard">
                                             <div class="card-body">
                                                  <img src="${newsObj.articles[key].urlToImage}" alt="news photo">
                                                  <h1 class="mx-3"> ${newsObj.articles[key].title}</h1>
                                             </div>
                                             <p>${newsObj.articles[key].description}
                                             <a href="${newsObj.articles[key].url}" target="_blank">View Ful Coverage</a></p>
                                             <h5 style="font-weight:600">News By : ${newsObj.articles[key].source.name}</h5>
                                        </div>
                                   </div>`;
               }
               
          }
          document.getElementById('newsCard').innerHTML = newsHtml;
     }else{
          console.log('Some error occured')
     }
}

xhr.send();
